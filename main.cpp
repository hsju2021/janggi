...
public:
    ...
    string letter2string() {
        if (this->letter == 'R' || this->letter == 'r')
            return "차";
        else if (this->letter == 'C' || this->letter == 'c')
            return "포";
        else if (this->letter == 'N' || this->letter == 'n')
            return "마";
        else if (this->letter == 'E' || this->letter == 'e')
            return "상";
        else if (this->letter == 'K' || this->letter == 'k')
            return "왕";
        else if (this->letter == 'G' || this->letter == 'g')
            return "사";
...
public:
    ...
    TurnTree(BoardState state) : root(new TurnTreeNode(state)) {
        currentNode = root;
    }

    void addNode(BoardState state) {
        if (currentNode == nullptr) {
            auto newNode = new TurnTreeNode(state);
            root = newNode;
            currentNode = root;
        }
        else {
        ...
        if ((this->x == 4) && (this->y == 1)) {  // 차가 가운데 있을 때 (4, 1)
            ...
            if (y <= 2) {  // 한나라
                for (int i = 0; i < 4; i++) {
                    ...
                    int y = starpointsHan[i][1];
                    if ((this->x == x) &&
                        (this->y == y)) {  // 모서리에 있는지 확인
                        if (isMovable(4, 1, this->team) ==
                            0) {  // 가운데 비었을 때
...
public:
    ...
    vector<pair<int, int>> generatePaths() override {  // 김종우 수정 사항
        ...
        for (int i = 0; i < 8; i++) {            // 8개 좌표 모두 검사
            ...
            if (x >= 0 && x < 9 && y >= 0 &&
                y < 10) {  // 지나가는 좌표가 판을 벗어나는지 검사
                if (isMovable(x, y, this->team) ==
                    0) {  // 지나가는 좌표에 기물이 없어야 다음 단계로 진행
                    x = this->x + moves[i][0];  // 최종 좌표 x, y 값 저장
                    y = this->y + moves[i][1];
                    if (x >= 0 && x < 9 && y >= 0 &&
                        y < 10) {  // 최종 좌표가 판을 벗어나는지 검사
                        if (isMovable(x, y, this->team) == 2)
                            ;  // 최종 좌표에 나의 기물이 있다면 다음 탐색 진행
                        else {
                            validMoves.push_back(make_pair(
                                x,
                                y));  // 빈 좌표나 적 기물이 있다면 경로에 저장
                        }
                    }
                }
            }
...
Piece* choosePiece(Player& player) {
    ...
    while (true) {
        ...
        if ((&player == &game.han && board[tmpx][tmpy]->team == 'C') ||
            (&player == &game.cho && board[tmpx][tmpy]->team == 'H')) {
...
void printBoard(int gamestart) {
    ...
    if (gamestart == 0) {
        ...
        for (int row = 0; row < 10; row++) {
            ...
            if (row == 0) {
                cout << " 한나라 score : " << game.han.score;
            }  // 첫째 줄에 한나라 score 출력
            else if (row == 1) {
                cout << " 초나라 score : " << game.cho.score;
            }  // 둘째 줄에 초나라 score 출력
            cout << endl;
        }
    }
...
bool choCheckWin() {  // 남경식
    if (isKingDie() || isTurnOver(game.turn) ||
...
// 남경식
bool isTurnOver(int turn) { return (game.turn >= 99); };
...
// 남경식
bool choKingDie(Piece* piece) { return (piece->team != 'C'); };

// 남경식
void undo() {
    if (tree->depth(tree->currentNode) > 1) {
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 10; j++) {
                board[i][j] = nullptr;
            }
...
// format(msg[내용], { {"바꿀문자열1", "바꾼후문자열1"}, {"바꿀문자열2",
// "바꾼후문자열2"}, ... })
string format(const string& input, const map<string, string>& to) {
    string result = input;
